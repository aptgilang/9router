...
export function buildOnStreamComplete({ provider, model, connectionId, apiKey, requestStartTime, body, stream, finalBody, translatedBody, clientRawRequest, pxpipe, reqTag, log, providerUrl, providerHeaders, sourceFormat, targetFormat }) {
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    const toolCalls = (contentObj?.tool_calls && contentObj.tool_calls.length > 0) ? contentObj.tool_calls : null;
    const hasContent = Boolean(contentObj?.content);
    const hasTools = Boolean(toolCalls);
    const hasThinking = Boolean(contentObj?.thinking);

    let safeContent = contentObj?.content || "";
    if (!hasContent && !hasTools && !hasThinking) {
